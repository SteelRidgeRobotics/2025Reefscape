...
class SwerveSetpointGenerator:
    ...
    def generateSetpoint(self, prev_setpoint: SwerveSetpoint, desired_state_robot_relative: ChassisSpeeds, dt: float,
        ...
        :param constraints: The arbitrary constraints to respect along with the robot's max capabilities.
        ...
        max_speed = self._config.moduleConfig.maxDriveVelocityMPS * min(1, input_voltage / 12)
        ...
        if constraints is not None:
            ...
            linear_vel = vel.norm()
            if linear_vel > constraints.maxVelocityMps:
                vel *= constraints.maxVelocityMps / linear_vel
            desired_state_robot_relative = ChassisSpeeds(
                vel.x, vel.y, max(
                    min(
                        desired_state_robot_relative.omega,
        ...
        SwerveDrive4Kinematics.desaturateWheelSpeeds(desired_module_states, max_speed)
        ...
        for m in range(self._config.numModules):
            ...
            chassis_force_vec += module_force_vec
        ...
        if constraints is not None:
            linear_accel = chassis_accel_vec.norm()
            if linear_accel > constraints.maxAccelerationMpsSq:
                chassis_accel_vec *= constraints.maxAccelerationMpsSq / linear_accel
            chassis_angular_accel = max(
                min(chassis_angular_accel, constraints.maxAngularAccelerationRpsSq),
    ...
    def _findDriveMaxS(
        ...
        def is_valid_s(s):
            return math.isfinite(s) and 0 <= s <= 1
